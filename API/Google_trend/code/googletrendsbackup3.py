...
def get_data(bucket_start_date, bucket_end_date, keyword):
    ...
    req = {"comparisonItem": [{"keyword": keyword, "geo": '',
                               "time": time_formatted}], "category": 7, "property": ""}
    hl = "en-GB"
    tz = "-120"
...
def get_weekly_frame(start_date, end_date, keyword):
    ...
    if datetime.strptime(start_date, '%Y-%m-%d') > datetime.strptime(end_date, '%Y-%m-%d') - timedelta(days=180):
        ...
        resp_text = get_data(datetime.strptime(
            start_date, '%Y-%m-%d'), datetime.strptime(end_date, '%Y-%m-%d'), keyword)
        return parse_csv(get_csv(resp_text))


def stitch_frames(daily_frames, weekly_frame, start_date, end_date, keywords, path=''):
    ...
    for i in range(0, len(weekly_frame)):
        bins.append(pd.date_range(
            weekly_frame['Week_Start_Date'][i], periods=7, freq='d'))
    ...
    for i in range(0, len(bins)):
        ...
        for j in range(0, len(bins[i])):
            ...
            try:
                this_val = int(float(weekly_frame['Weekly_Volume'][week_start_date]) * float(daily_frame[
                               'Daily_Volume'][this_date]) / float(daily_frame['Daily_Volume'][week_start_date]))
                final_data[this_date] = this_val
            except:
                pass
    final_data_frame = DataFrame.from_dict(
        final_data, orient='index').sort_index()
    final_data_frame[0] = np.round(
        final_data_frame[0] / final_data_frame[0].max() * 100, 2)

    final_data_frame.columns = [keywords]
    final_data_frame.index.names = ['Date']
    filename = '+'.join(keywords) + '_' + str(start_date) + '-' + str(end_date)
    if path != '':
        final_data_frame.to_csv(path + '{0}.csv'.format(
            filename.replace('+', '')), sep=',')

    else:

        final_data_frame.to_csv('{0}.csv'.format(
            filename.replace('+', '')), sep=',')
...
if __name__ == '__main__':
    ...
    stitch_frames(daily_frames, weekly_frame, start_date,
                  end_date, keywords=keywords)
