...
def stitch_frames(daily_frames, weekly_frame):
    ...
    path = ''
    if path != '':
        final_data_frame.to_csv(path + '{0}.csv'.format(filename), sep=',')

    else:
        final_data_frame.to_csv('{0}.csv'.format(filename), sep=',')

        # filename = keywords + '_' + str(start_date) + '-' + str(end_date)

        # final_data_frame.to_csv('{0}.csv'.format(filename.replace('+', '')), sep=',')
