...
class TrendReq(object):
    ...
    def build_payload(self, kw_list, cat=7, timeframe="today 5-y", geo='', gprop=''):
    ...
    def interest_over_time(self):
        ...
        over_time_payload['req'] = json.dumps(
            self.interest_over_time_widget['request'])
        ...
        # split list columns into seperate ones, remove brackets and split on
        # comma
        result_df = df['value'].apply(lambda x: pd.Series(
            str(x).replace('[', '').replace(']', '').split(',')))
        # rename each column with its search term, relying on order that google
        # provides...
    ...
    def related_queries(self):
        ...
        for request_json in self.related_queries_widget_list:
            # ensure we know which keyword we are looking at rather than
            # relying on order
            ...
            req_json = json.loads(req.text[5:])
            # top queries
            top_df = pd.DataFrame(req_json['default'][
    ...
    def suggestions(self, keyword):
        ...
        req = self.ses.get(
            "https://www.google.com/trends/api/autocomplete/" + kw_param)

        # parse the returned json
        # response is invalid json but if you strip off ")]}'," from the front
        # it is then valid
