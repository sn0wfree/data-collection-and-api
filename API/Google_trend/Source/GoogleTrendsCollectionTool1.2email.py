...
def progress_test(counts, lenfile, speed):
    ...
    sys.stdout.flush()
def AppendZipFile(target_file,tagret_zip_file):
        z = zipfile.ZipFile(tagret_zip_file, 'a')
        z.write('target_file')
        z.close()
...
if __name__ == '__main__':
    ...
    target = locals_file_path + link + '3.txt'
    target_data_dir= locals_file_path+link+'data'
    csvfiles =[ file for file in os.listdir(locals_file_path) if '.csv' in target_data_dir]
            ...
            try:
                ...
                if request_time > 60:
                    if mail == 1:
                        ...
                        now =  datetime.today()
                        pathfile=locals_file_path+ link+'%s.zip' % now
                        print 'compress process completed'
                        zipafilefordelivery(pathfile,csvfiles)
                        AppendZipFile(target_file,pathfile)
    ...
    if mail == 1:
        ...
        to_addrs="snowfreedom0815@gmail.com"
        attachmentFilePaths=pathfile
        sendmailtodelivery(mail_username,'Linlu19920815',to_addrs,attachmentFilePaths)
        print 'email sent'
    else:
        print 'TAT, coding is tough job for non-computer science students'                            
