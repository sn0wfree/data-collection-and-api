...
class GTsCTool():
    ...
    def __init__(self, keywords, custom_useragent=None):
        self.dateFrame = []
        self.bucket_limits = []
        self.keywords = keywords
        custom_useragent = 'Mozilla/5.0 (Windows NT 6.3; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0'

        # google_username, google_password, hl='en-US', tz=360, geo='',
        # ):
        """
        ...
        # custom user agent so users know what "new account signin for Google"
        # is
    ...
    def get_buckets(self, start_date, end_date):
        start_date_dt = datetime.strptime(start_date, '%Y-%m-%d')
        end_date_dt = datetime.strptime(end_date, '%Y-%m-%d')
        self.bucket_limits = [start_date_dt]
        left_limit = start_date_dt
        while left_limit <= end_date_dt:
            new_limit = left_limit + timedelta(days=365)
            if new_limit < end_date_dt:
                self.bucket_limits.append(new_limit)
            left_limit = new_limit
        self.bucket_limits.append(end_date_dt)
        return self.bucket_limits

    def get_data(self, bucket_start_date, bucket_end_date, keywords):
        bucket_start_date_printed = datetime.strftime(
            bucket_start_date, '%Y-%m-%d')
        bucket_end_date_printed = datetime.strftime(
            bucket_end_date, '%Y-%m-%d')
    ...
    def HandleGTs(self, start_date, end_date, keywords):
        start_date_dt = datetime.strptime(start_date, '%Y-%m-%d')
        end_date_dt = datetime.strptime(end_date, '%Y-%m-%d')
        resp_text = self.get_data(start_date_dt, end_date_dt, keywords)
        df = self.parse_csv(self.get_csv(resp_text), keywords)
        return df

    def get_csv(self, response_text):
        try:
            request = response_text.split('"widgets":')[1].split(
                ',"lineAnno')[0].split('"request":')[1]

            # print request
            token = self.get_token(response_text)
            ...
            raise 'a Error to get csv'

    def get_token(self, response_text):
        try:
            return response_text.split('token":"')[1].split('","')[0]
        except:
            raise 'a Error to get token'
            return None

    def parse_csv(self, csv_contents, keywords):
...
def _connect(google_username, google_password, custom_useragent=None):
    """
    Connect to Google.
    Go to login page GALX hidden input value and send it back to google + login and password.
    http://stackoverflow.com/questions/6754709/logging-in-to-google-using-python
    ...
    # is
    if custom_useragent is None:
        custom_useragent = {'User-Agent': 'PyTrends'}
    else:
        custom_useragent = {'User-Agent': custom_useragent}

    google_rl = 'You have reached your quota limit. Please try again later.'
    url_login = "https://accounts.google.com/ServiceLogin"
    url_auth = "https://accounts.google.com/ServiceLoginAuth"
    login_html = ses.get(url_login, headers=custom_useragent)
    ...
    for u in soup_login:
        if u.has_attr('value') and u.has_attr('name'):
            form_data[u['name']] = u['value']
    # override the inputs with out login and pwd:
    form_data['Email'] = google_username
    form_data['Passwd'] = google_password
    ses.post(url_auth, data=form_data)
...
def progress_test(counts, lenfile, speed):
    ...
    # sys.stdout.write("\rthis spider has already read %d projects, speed:
    # %.4f/projects" % (counts,f2-f1))

    # sys.stdout.write("\rPercent: [%s] %d%%,remaining time: %.4f
    # mins"%(hashes + spaces,precent,w))
    sys.stdout.flush()
...
if __name__ == '__main__':
    ...
    # da_temp =
    # '/Users/sn0wfree/Documents/python_projects/data_collection/API/Google_trend/'
    ...
    _connect(google_username, google_password)
