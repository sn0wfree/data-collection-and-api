...
class xml2sql:
    ...
    def convert(self, tag="title", table="table", ignore=[], limit=-1, packet=8):
        ...
        for event, elem in self.context:
            # if elem is an unignored child node of the record tag, it should
            ...
            if event == 'start':
                if elem.tag == tag and not started:
                    ...
                    # if elem is nested inside a "parent", field name becomes
                    # parent_elem
                    field_name = '_'.join(
                        (field_name, elem.tag)) if field_name else elem.tag

            else:
                if should_write:
                    if should_tag:
                        # add field name to csv header
                ...
                # end of traversing the record tag
                    ...
                    if sql_len + len(self.sql_insert) + 100 < max_packet:
                        ...
                        # packet size exceeded. flush the sql and start a new
                        # insert query
                        self._write_buffer()
                        self.output_buffer.append(sql)
                        sql_len = 0
